...
constexpr uint64_t SAMPLES = 100;

constexpr uint64_t bytes_upper_bound = 8388608;
constexpr uint64_t NUM_RUNS = 2000;
...
static u64 getRand(u64 min, u64 max, uint64_t& seed)
{
   u64 rand = min + (rnd(seed) % (max - min));
   assert(rand < max);
   assert(rand >= min);
   return rand;
}

void getRandString(u8* dst, u64 size, uint64_t& seed)
{
   for (u64 t_i = 0; t_i < size; t_i++) {
      dst[t_i] = getRand(48, 123, seed);
   }
}
