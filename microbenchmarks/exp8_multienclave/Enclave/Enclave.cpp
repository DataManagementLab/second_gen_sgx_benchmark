...
struct BTree {
   ...
   uint64_t getPageCount() { return page_count; }
...
void ecall_run(uint64_t* ops, uint64_t* inserts, uint64_t* lookups, uint64_t* page_count)
{
   using Key = uint64_t;
   using Value = uint64_t;
   btreeolc::BTree<Key, Value> tree;
   constexpr Key upper_bound_key = 507270828;  // not sure if needed ?
   Key highest_key = 0;                        // tracks highest inserted key sofar

   constexpr uint64_t read_ratio = 80;
   ...
   // fill initial b-tree with 100
   for (Key k_i = 0; k_i < 100; k_i++) {
      tree.insert(k_i, k_i);
      highest_key++;
   }

   for (int o_i = 0; o_i < 50e9; o_i++) {
      // read
      ...
      if (operation < read_ratio) {
         ...
         tree.insert(highest_key, highest_key);
         highest_key++;
         (*inserts_v)++;
      }
      (*ops_v)++;
